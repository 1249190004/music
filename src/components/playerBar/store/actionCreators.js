...
export const changeCurrentIndexAndSongAction = (tag) => {
  return (dispatch, getState) => {
    const playList = getState().getIn(["playMusic", "playList"]);
    const sequence = getState().getIn(["playMusic", "sequence"]);
    let currentSongIndex = getState().getIn(["playMusic", "currentSongIndex"]);
...
export const getPlayListAction = (playlist) => {
  return (dispatch, getState) => {
    const playList = getState().getIn(["playMusic", "playList"]);

    dispatch(changePlayListAction([...playlist,...playList]));
